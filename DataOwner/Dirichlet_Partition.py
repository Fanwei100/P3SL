"""Dirichlet-based non-IID dataset partitioner.

Builds heterogeneous per-client splits of a torchvision dataset by drawing
class proportions from a Dirichlet distribution parameterised by ``alpha``.
Smaller ``alpha`` produces more skewed (less IID) partitions, which is the
common setting for benchmarking federated split-learning algorithms.
"""
